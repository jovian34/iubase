'''
Original code for this module generated by ChatGPT
Carl James performed basic refactoring after the fact
'''
...
def build_crpi_rows(teams, spring_year: int) -> List[dict]:
    all_series, team_ids = load_all_series(teams, spring_year)
    pair_data, pair_split = build_lookups(all_series, team_ids)
    opponents_map = build_opponent_map(team_ids, pair_data)
    team_records = build_raw_records(team_ids, pair_data, opponents_map)
    adjusted_records = adjust_records(team_ids, pair_split, opponents_map)
    opponent_win_pct = build_opponent_win_percentage(team_ids, opponents_map, team_records)
    opponent_opponent_win_pct = build_opp_opponent_win_percentage(team_ids, opponents_map, team_records)
    rows = build_final_rows(teams, team_records, adjusted_records, opponent_win_pct, opponent_opponent_win_pct)
    rows.sort(key=lambda r: (-r["crpi"], r["team_name"].casefold()))
    return rows


def load_all_series(teams, spring_year):
    all_series = list(
        conf_models.ConfSeries.objects.filter(start_date__year=spring_year)
    )

    team_ids = {team.pk for team in teams}
    return all_series,team_ids


def build_lookups(all_series, team_ids):
    ...
    return pair_data,pair_split


def build_opponent_map(team_ids, pair_data):
...
def build_raw_records(team_ids, pair_data, opponents_map):
    team_records = {}
...
def adjust_records(team_ids, pair_split, opponents_map):
    adjusted_records = {}
...
def build_opponent_win_percentage(team_ids, opponents_map, team_records):
    opponent_win_pct = {}
    ...
    return opponent_win_pct


def build_opp_opponent_win_percentage(team_ids, opponents_map, team_records):
    ...
    return opponent_opponent_win_pct


def build_final_rows(teams, team_records, adjusted_records, opponent_win_pct, opponent_opponent_win_pct):
    rows = []
