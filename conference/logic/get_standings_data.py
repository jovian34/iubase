...
def annotated_teams_queryset(spring_year):
    rpi_subquery = conf_models.TeamRpi.objects.filter(
        team=OuterRef("pk"), spring_year=spring_year
    ).values("rpi_rank")[:1]

    home_wins_sq = (
        conf_models.ConfSeries.objects.filter(
            home_team=OuterRef("pk"), start_date__year=spring_year
        )
        .values("home_team")
        .annotate(total=Sum("home_wins"))
        .values("total")[:1]
    )

    away_wins_sq = (
        conf_models.ConfSeries.objects.filter(
            away_team=OuterRef("pk"), start_date__year=spring_year
        )
        .values("away_team")
        .annotate(total=Sum("away_wins"))
        .values("total")[:1]
    )
...
def limit_to_teams_with_B1G_rpi_and_games_this_season(teams_qs):
    teams_qs = teams_qs.filter(rpi_rank__isnull=False).filter(
        Q(wins__gt=0) | Q(losses__gt=0)
    )
    return teams_qs
