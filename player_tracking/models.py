...
from player_tracking.choices import HAND_CHOICES, TRANSACTION_CHOICES
...
class Player(models.Model):
    ...
    throws = models.CharField(choices=HAND_CHOICES, max_length=16, null=True, blank=True)
...
class Transaction(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.player.first} {self.player.last} {self.trans_event} on {self.trans_date}"

class AnnualRoster(models.Model):
    fall_year = models.IntegerField(null=False)
    player = models.ForeignKey(Player, on_delete=models.CASCADE)
    jersey_number = models.IntegerField(null=True, blank=True)
    spring_roster = models.BooleanField(null=True, blank=True)
    played_in_spring = models.BooleanField(null=True, blank=True)
    grey_shirt = models.BooleanField(null=True, blank=True)
    granted_waiver = models.BooleanField(null=True, blank=True)

    def __str__(self) -> str:
        return f"{self.player.first} {self.player.last} {self.fall_year}"
