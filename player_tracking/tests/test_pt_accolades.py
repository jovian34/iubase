...
@pytest.mark.django_db
def test_accolade_page_shows_accolade(client, accolades, players, annual_rosters, teams):
    response = client.get(urls.reverse("accolades"))
    assert "Pre-season second team All-American Outfielder" in str(response.content)

@pytest.mark.django_db
def test_accolade_page_shows_college_team_if_not_indiana(client, accolades, players, annual_rosters, teams):
    response = client.get(urls.reverse("accolades"))
