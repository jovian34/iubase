...
@pytest.mark.django_db
def test_portal_page_shows_date_entered(client, players, teams, annual_rosters, transactions):
    response = client.get(reverse("portal", args=[str(this_year)])) 
    assert response.status_code == 200
    assert f"Entered the portal</a> on June 15, {str(this_year)}" in str(response.content)


@pytest.mark.django_db
def test_portal_page_links_to_citation(client, players, teams, annual_rosters, transactions):
    response = client.get(reverse("portal", args=[str(this_year)])) 
    assert response.status_code == 200
    assert '<a href="https://x.com/brooks_ey/status/1794162572117303480">' in str(response.content)


@pytest.mark.django_db
def test_portal_page_shows_date_commited(client, players, teams, annual_rosters, transactions):
