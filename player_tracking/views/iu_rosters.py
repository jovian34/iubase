...
from player_tracking.models import AnnualRoster, Accolade
...
def spring(request, spring_year):
    annual_rosters = (
        AnnualRoster.objects.filter(spring_year=int(spring_year))
        .filter(team__team_name="Indiana")
        .filter(status__in=choices.ALL_ROSTER)
        .order_by("jersey")
    )
    accolades = Accolade.objects.filter(annual_roster__in=annual_rosters)
    if len(annual_rosters) < 5 and int(spring_year) >= date.today().year:
        page_title = f"Spring {spring_year} Roster not fully announced"
    else:
        page_title = f"Spring {spring_year} Roster"
    context = {
        "players": annual_rosters,
        "page_title": page_title,
        "total": len(annual_rosters),
        "accolades": accolades,
    }
